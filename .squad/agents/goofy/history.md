...
- 2026-05-28 -- #461 audit (research pass): Grepped entire repo for $IsWindows/$IsLinux/$IsMacOS/$IsCoreCLR. Found 4 occurrences in setup.ps1 (all guarded with PSVersion short-circuit -- SAFE), 5 in test_windows_setup.ps1 (guarded with Test-Path Variable: or PSVersion -- SAFE), 0 in any other file currently. The original bare `if (-not $IsWindows)` bug (commit 550821a) was already fixed in PR #462 (commit 31aa228) via [System.Environment]::OSVersion.Platform. $IsCoreCLR: zero occurrences in repo. Recommended primary detection: `[System.Environment]::OSVersion.Platform -ne [System.PlatformID]::Win32NT` (PS 5.1 safe, enum type-safe, already adopted). Skill extracted: .squad/skills/ps51-platform-detection-audit/SKILL.md.
