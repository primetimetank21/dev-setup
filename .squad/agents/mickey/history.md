...
- 2026-05-30: v9 #468 plan -- Invoke-WingetGate fail-fast fix + -Skip winget-check foot-gun doc (f4bea92)
- 2026-05-28: v13 #468 plan -- Surgical fix: line 167 DD-1 used `--only=uv` in a "fresh Windows" example (Linux syntax in Windows context). Duck-2 caught on v12 regrill after Doc/Jiminy/Goofy all missed it. Fixed to `-Only 'uv'`. Sweep clean: all `--only=`/`--skip=` confirmed Linux-context-only; all `-Only '...'`/`-Skip '...'` confirmed Windows-context-only.
