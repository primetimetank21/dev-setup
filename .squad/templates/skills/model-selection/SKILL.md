...
7. INCLUDE model in spawn acknowledgment: `[TOOL] {Name} ({resolved_model}) -- {task}`
...
3. ACKNOWLEDGE: `[x] Model preference saved: {model} -- all future sessions will use this until changed.`

**Per-agent trigger:** "use X for {agent}"

1. VALIDATE model ID
2. WRITE to `agentModelOverrides.{agent}` in `.squad/config.json`
3. ACKNOWLEDGE: `[x] {Agent} will always use {model} -- saved to config.`
...
2. ACKNOWLEDGE: `[x] Model preference cleared -- returning to automatic selection.`
