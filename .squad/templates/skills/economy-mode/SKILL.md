...
- Spawn acknowledgments with `[COST]` indicator when economy mode is active
...
2. ACKNOWLEDGE: `[x] Economy mode active -- using cost-optimized models this session. (Layer 0 and Layer 2 preferences still apply)`
...
2. ACKNOWLEDGE: `[x] Economy mode saved -- cost-optimized models will be used until disabled.`
...
5. INCLUDE `[COST]` in spawn acknowledgment: `[TOOL] {Name} ({model} * [COST] economy) -- {task}`
...
3. ACKNOWLEDGE: `[x] Economy mode disabled -- returning to standard model selection.`

### STOP

After updating economy mode state and including the `[COST]` indicator in spawn acknowledgments, this skill is done. Do NOT:
