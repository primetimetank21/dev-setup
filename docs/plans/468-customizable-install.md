...
# Extract Linux defaults from DEFAULT_TOOLS array in source
linux_defaults=$(grep -A50 '^DEFAULT_TOOLS=(' "$REPO_ROOT/scripts/linux/setup.sh" \
  | sed -n '/^DEFAULT_TOOLS=(/,/^)/p' | grep '"' | sed 's/.*"\(.*\)".*/\1/')
echo "$linux_defaults" > "$REPO_ROOT/tests/fixtures/baseline-tools-linux.txt"

# Extract Windows defaults from $DefaultTools array in source
pwsh -Command "
  \$content = Get-Content '$REPO_ROOT/scripts/windows/setup.ps1' -Raw
  if (\$content -match '(?s)\\\$DefaultTools\s*=\s*@\((.*?)\)') {
    \$Matches[1].Split([char[]]@([char]13,[char]10,' ')) |
      ForEach-Object { \$_.Trim().Trim(\"'\") } |
      Where-Object { \$_ } |
      Set-Content '$REPO_ROOT/tests/fixtures/baseline-tools-windows.txt' -Encoding ascii
  }
"
...
```
# Bash
T_skip_single:            --skip=bravo installs all DEFAULT_TOOLS except bravo
...
# PowerShell
T_skip_single:            -Skip 'bravo' installs all DefaultTools except bravo
T_skip_multi:             -Skip 'bravo,charlie' excludes both
T_skip_unknown:           -Skip 'bogus' exits 1
T_skip_empty:             -Skip '' exits 1
T_skip_conflict:          -Only 'alpha' -Skip 'bravo' exits 1
...
- `.github/workflows/validate.yml` -- add `--list` and `--only` smoke steps

**e2e Scenarios (in CI workflow):**

```
E2E_list_linux:   ./setup.sh --list exits 0 (ubuntu + macos)
E2E_list_win:     .\setup.ps1 -List exits 0 (windows)
E2E_only_linux:   ./setup.sh --only=gh exits 0 and gh available (ubuntu)
E2E_only_win:     .\setup.ps1 -Only "gh" exits 0 and gh available (windows)
```

**Done Criteria:**
- [ ] README documents all flags with examples
- [ ] README notes that `--only` does not auto-include prerequisites
- [ ] e2e exercises `--list` + `--only` on all platforms via root entrypoints
